--[[
    SCBI LibreScript v1.7 - A liberated script for SimCity BuildIt
    Copyright (C) 2026
...
if not shouldSkipMenu() then
	local opt = gg.alert(
		[[
⚠️ 64-BITS SCRIPT ONLY - LIBRESCRIPT v1.7
...
function UnifiedBuildingSearch()
	if BUILDING_CACHE.isLoaded then
		return true
	end
	gg.setVisible(false)
	...
	if #results == 0 then
		gg.alert("Error: Building base not found.")
		return false
	end

	local bestListAddr = nil
	local maxMatches = 0

	for _, res in ipairs(results) do
		local ptr1 = gg.getValues({ { address = res.address + 0x40, flags = gg.TYPE_QWORD } })[1].value
		if ptr1 and ptr1 ~= 0 then
			local ptr2 = gg.getValues({ { address = ptr1 + 0x4, flags = gg.TYPE_QWORD } })[1].value
			if ptr2 == 7235441155891999603 then
				local sourceAddr = res.address - 0x10
				gg.clearResults()
				gg.searchNumber(string.format("%Xh", sourceAddr), gg.TYPE_QWORD)
				local listPointers = gg.getResults(gg.getResultsCount())

				for _, lp in ipairs(listPointers) do
					local currentListStart = lp.address
					local testItems = {}
					for j = 0, 3000 do
						table.insert(testItems, { address = currentListStart + (j * 8), flags = gg.TYPE_QWORD })
					end

					local testPtrs = gg.getValues(testItems)
					local idsToRead = {}
					local ptrMap = {}

					for _, tp in ipairs(testPtrs) do
						if tp.value ~= 0 and tp.value > 0x100000000 then
							table.insert(idsToRead, { address = tp.value + 0x10, flags = gg.TYPE_DWORD })
						end
					end

					if #idsToRead > 0 then
						local idResults = gg.getValues(idsToRead)
						local currentMatchCount = 0
						for _, idRes in ipairs(idResults) do
							if buildingNameById[idRes.value] or PYRAMID_BUILDING_MAP[idRes.value] then
								currentMatchCount = currentMatchCount + 1
							end
						end

						if currentMatchCount > maxMatches then
							maxMatches = currentMatchCount
							bestListAddr = currentListStart
						end
					end
				end
			end
	...
	if not bestListAddr or maxMatches < 3 then
		gg.alert("Error: No valid building list identified.")
		return false
	end

	local fullListRead = {}
	for i = 0, 5500 do
		table.insert(fullListRead, { address = bestListAddr + (i * 8), flags = gg.TYPE_QWORD })
	end

	local allPointers = gg.getValues(fullListRead)
	local finalIdsToRead = {}
	local validPtrs = {}

	for i, ptr in ipairs(allPointers) do
		if ptr.value and ptr.value ~= 0 and ptr.value > 0x100000000 then
			table.insert(finalIdsToRead, { address = ptr.value + 0x10, flags = gg.TYPE_DWORD })
			table.insert(validPtrs, ptr.value)
		else
			if i > 5 then
				break
			end
	...
	if #finalIdsToRead == 0 then
		return false
	end

	local finalIdValues = gg.getValues(finalIdsToRead)
	for i, idVal in ipairs(finalIdValues) do
		BUILDING_CACHE.data[idVal.value] = validPtrs[i]
		BUILDING_CACHE.orderedList[i] = validPtrs[i]
	end

	BUILDING_CACHE.isLoaded = true
	BUILDING_CACHE.count = #finalIdValues
	gg.toast("Database Synced: " .. BUILDING_CACHE.count .. " items (Confidence: " .. maxMatches .. ")")
	return true
end
...
function GlobalFactoryRevert(silent)
	if #FACTORY_MANAGER.slots == 0 then
		return
	end
	local toRevert = {}
	for i = 1, #FACTORY_MANAGER.slots do
		local slot = FACTORY_MANAGER.slots[i]
		table.insert(toRevert, { address = slot.address, flags = gg.TYPE_QWORD, value = slot.initialValue })
		slot.currentName = nil
	end
	gg.setValues(toRevert)
	if not silent then
		gg.toast("Factory Reverted")
	end
end

function EnsureFactorySlots(count)
	if #FACTORY_MANAGER.slots >= count then
		return true
	end
	gg.setVisible(false)
	local start = #FACTORY_MANAGER.slots + 1
	for i = start, count do
		gg.clearResults()
		gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_OTHER | gg.REGION_ANONYMOUS)
		gg.searchNumber(FACTORY_MANAGER.constants[i], gg.TYPE_DWORD)
		local res = gg.getResults(1)
		if #res > 0 then
			local addr = res[1].address + 0x1C
			local val = gg.getValues({ { address = addr, flags = gg.TYPE_QWORD } })[1].value
			FACTORY_MANAGER.slots[i] = { address = addr, initialValue = val, currentName = nil }
		else
			gg.alert("Error: Slot " .. i .. " not found.")
			return false
		end
	end
	return true
...
local VU_PASS_CELLS = nil
local VALID_CELL_VALUES = { [1200] = true, [14500] = true, [42000] = true, [97000] = true }

function GetVuPassBases()
	if VU_PASS_CELLS then
		gg.toast("Using cached Vu Pass cells...")
		return VU_PASS_CELLS
	end
	...
	if #results == 0 then
		gg.alert("❌ Vu Pass bases not found. Open Vu Pass menu first.")
		gg.setVisible(true)
		return nil
	end
	...
	for _, r in ipairs(results) do
		...
		local ptrValRes = gg.getValues({ { address = ptrAddr, flags = gg.TYPE_QWORD } })
		if ptrValRes[1].value ~= 0 then
			local val180Res = gg.getValues({ { address = ptrValRes[1].value + 0x180, flags = gg.TYPE_DWORD } })
			if val180Res[1].value == 1145656354 then
				local pointer1Res = gg.getValues({ { address = val180Res[1].address - 0xF8, flags = gg.TYPE_QWORD } })
				if pointer1Res[1].value ~= 0 then
					local p1 = pointer1Res[1].value
					local cellPointers = gg.getValues({
						{ address = p1, flags = gg.TYPE_QWORD },
						{ address = p1 + 0x18, flags = gg.TYPE_QWORD },
						{ address = p1 + 0x30, flags = gg.TYPE_QWORD },
						{ address = p1 + 0x48, flags = gg.TYPE_QWORD },
					})
					for _, cp in ipairs(cellPointers) do
						if cp.value ~= 0 then
							local check = gg.getValues({ { address = cp.value + 0xC, flags = gg.TYPE_DWORD } })
							if check[1].value and VALID_CELL_VALUES[check[1].value] then
								table.insert(baseAddresses, cp.value + 0x30)
							end
						end
					end
					if #baseAddresses == 4 then
						VU_PASS_CELLS = baseAddresses
						gg.toast("✅ 4 Cells Cached Successfully!")
						gg.setVisible(true)
						return VU_PASS_CELLS
					end
				end
...
local populationEnabled = false
local populationValue = nil
local originalPopulation = {}
local editedPopulationAddresses = {}
function Population()
	if populationEnabled then
		local restore = {}

		for i = 1, #editedPopulationAddresses do
			restore[#restore + 1] = {
				address = editedPopulationAddresses[i],
				flags = gg.TYPE_DWORD,
				value = originalPopulation[i],
			}
		end
		...
		populationValue = nil
		originalPopulation = {}
		editedPopulationAddresses = {}

		gg.toast("Population restored.")
		return
	end
	...
	for i = 1, #anchors do
		local checkAddr = anchors[i].address + 0x10
		...
		if check == 15 then
			local keyAddr = checkAddr - 0x20

			keyToSearch = gg.getValues({
				{ address = keyAddr, flags = gg.TYPE_DWORD },
			})[1].value
	...
	if not keyToSearch then
		gg.toast("No valid 15 marker found.")
		return
	end
	...
	for i = 1, #results do
		local addr = results[i].address + 0xA8

		local original = gg.getValues({
			{ address = addr, flags = gg.TYPE_DWORD },
		})[1].value

		originalPopulation[#originalPopulation + 1] = original
		editedPopulationAddresses[#editedPopulationAddresses + 1] = addr
	...
	populationValue = desiredPopulation
...
local CONFIG = {
	{ name = "🪙 Simoleons", offsets = { 0x8, 0xc, 0x10, 0x14 }, active = false },
...
function findPointerBase()
	...
	gg.searchNumber("1729378059939061760", gg.TYPE_QWORD)
...
local currencyData = {
	...
	monaco_ticket = {
		...
		hexCodes = {
			block1 = { 0x6D616320, 0x67696170, 0x7275636E, 0x636E6572, 0x00000079, 0x00000000 },
			block2 = { 0x7A614A12, 0x696F437A, 0x0000736E, 0x00000000, 0x00000000, 0x00000000 },
		},
...
function mainMenuO()
	cachedBaseAddresses = GetVuPassBases()
...
local function fixHexBlocks(currency)
	if currency.isSpecial then
		currency.hexCodes.block1 = currency.hexCodes.block1 or {}
		currency.hexCodes.block2 = currency.hexCodes.block2 or {}

		for i = 1, 6 do
			currency.hexCodes.block1[i] = currency.hexCodes.block1[i] or 0
			currency.hexCodes.block2[i] = currency.hexCodes.block2[i] or 0
		end
	end
end
...
local function applyContestRankRewards(baseAddress, selectedCurrencyKeys, amounts)
				...
				if ptr1 and ptr1 ~= 0 and ptr2 and ptr2 ~= 0 then
					...
					for j = 1, 6 do
						table.insert(allEdits, {
							address = ptr1 + (j - 1) * 4,
							flags = gg.TYPE_DWORD,
							value = currency.hexCodes.block1[j] or 0,
						})

						table.insert(allEdits, {
							address = ptr2 + (j - 1) * 4,
							flags = gg.TYPE_DWORD,
							value = currency.hexCodes.block2[j] or 0,
						})
					end

					table.insert(
						allEdits,
						{ address = qty_addr, flags = gg.TYPE_DWORD, value = amounts[currentKey] or 0 }
					)
	...
	for i, v in ipairs(allEdits) do
		if v.value == nil then
			gg.alert("Erro detectado: valor nil no índice " .. i)
			return
		end
	end
...
function findAndCacheProductionGroup()
	...
	for i, r in ipairs(results) do
		minus48[i] = { address = r.address + 0x10, flags = gg.TYPE_DWORD }
	end
	minus48 = gg.getValues(minus48)
	local only16 = {}
	for _, v in ipairs(minus48) do
		if v.value == 16 then
			table.insert(only16, { address = v.address - 0x20, flags = gg.TYPE_DWORD })
		end
	end
	if #only16 == 0 then
	...
	for _, r in ipairs(finalResults) do
		local prodTimeAddr = r.address + 0xA8
		local baseAddr = prodTimeAddr - 0x88
		local checkVal = gg.getValues({ { address = baseAddr, flags = gg.TYPE_DWORD } })[1].value

		if checkVal == 16 then
			table.insert(cachedXPAddresses, baseAddr + 0x220)
		end
	end
...
function applyProductionMultiplier(quantity, targetItemID)
	...
	for _, base in ipairs(cachedProductionGroup) do
		local pointerAddress = base.address + 0x220
...
function toggleProductionTime()
	...
	originalProductionTimes = {}
	for _, base in ipairs(cachedProductionGroup) do
		local address = base.address + 0xA8
...
function toggleRemoveNeed()
	...
	for _, result in ipairs(cachedProductionGroup) do
		...
		table.insert(addressesToRead, { address = sAddr + 0x200, flags = gg.TYPE_DWORD })
		table.insert(addressesToRead, { address = sAddr + 0x208, flags = gg.TYPE_DWORD })
	end
...
function applyPreset(presetName)
	local buildingsInPreset = warpresets[presetName]
	...
	GlobalFactoryRevert(true)
	local count = #buildingsInPreset
	if not EnsureFactorySlots(count) then
		return
	end
	local changes = {}
	for i, building in ipairs(buildingsInPreset) do
		local info = buildingDatabase[building.id]
		if info then
			table.insert(
				changes,
				{ address = FACTORY_MANAGER.slots[i].address, flags = gg.TYPE_QWORD, value = info.pointer }
			)
			FACTORY_MANAGER.slots[i].currentName = info.name
		end
	end
	if #changes > 0 then
		gg.setValues(changes)
	end
	gg.alert("✅ Preset '" .. presetName .. "' applied!")
end

function initializeScript()
	if not UnifiedBuildingSearch() then
		return false
	end
	buildingDatabase = {}
	for id, ptr in pairs(BUILDING_CACHE.data) do
		if PYRAMID_BUILDING_MAP[id] then
			buildingDatabase[id] = { name = PYRAMID_BUILDING_MAP[id], pointer = ptr }
		end
	end
	isInitialized = true
	return true
...
function showBoosterInfo()
	...
	for i = 1, 7 do
		...
		if cacheItem and cacheItem.currentBuildingId then
			...
			for presetName, buildingList in pairs(warpresets) do
...
function performDirectConversion(conversionMap)
	if not isInitialized then
		if not initializeScript() then
			gg.alert("Fail. It was not possible to convert.")
			return
		end
	end
	...
	for _, conversion in ipairs(conversionMap) do
		...
		if buildingInfo and buildingInfo.pointer then
			table.insert(changesToApply, {
				address = buildingInfo.pointer + 0x10,
				value = boosterId,
				flags = gg.TYPE_DWORD,
			})
	...
	while true do
		...
		if choice == nil then
			...
			if selectedOption == "💥 Convert ALL BOOSTERS 💥" then
				local conversionMap = {}
				for presetName, buildings in pairs(warpresets) do
...
Rail Simoleons: (SAME AS YOUR WALLET MAX LIMIT)  
...
buildingNameById = {
	...
	[1691892761] = "Floating Villa",
	[-1872807652] = "Coral islands",
	[-1980271564] = "Aquatic Mall",
	[2004125040] = "Aquatic Observation Deck",
	[-1980163556] = "Aquatic Park",
	[-1675500681] = "Aquatic Restaurant",
	[-694372480] = "River Delta",
	[1257884349] = "Submarine Tour",
	[1846917630] = "Aquatic Water Park",
	[1966805856] = "Aquatic Bungalows",
	[-108883065] = "Turtle Sunctuary",
	[-843811902] = "Aquatic Dome Resort (Level 1)",
	[-843811901] = "Aquatic Dome Resort (Level 2)",
	[-843811900] = "Aquatic Dome Resort (Level 3)",
	[-843811899] = "Aquatic Dome Resort (Level 4)",
	[-843811898] = "Aquatic Dome Resort (Level 5)",
	[-843811897] = "Aquatic Dome Resort (Level 6)",
	[-843811896] = "Aquatic Dome Resort (Level 7)",
	[-843811895] = "Aquatic Dome Resort (Level 8)",
	[-843811894] = "Aquatic Dome Resort (Level 9)",
	[-2075988942] = "Aquatic Dome Resort (Level 10)",
	[-2075988941] = "Aquatic Dome Resort (Level 11)",
	[-2075988940] = "Aquatic Dome Resort (Level 12)",
	[-2075988939] = "Aquatic Dome Resort (Level 13)",
	[-2075988938] = "Aquatic Dome Resort (Level 14)",
	[-2075988937] = "Aquatic Dome Resort (Level 15)",
	[-2075988936] = "Aquatic Dome Resort (Level 16)",
	[-2075988935] = "Aquatic Dome Resort (Level 17)",
	[-2075988934] = "Aquatic Dome Resort (Level 18)",
	[-669830354] = "K-Pop Park Avenue",
	[712194980] = "Cherry Gazebo",
	[905448001] = "Deoksugsung Palace",
	[-164981350] = "Changdeokgung Palace",
	[-1107023493] = "Hyangwonjeong Pavilion",
	[1453854792] = "Bongeunsa Temple",
	[-2136646598] = "Cherry Park",
	[1832050374] = "Children's Park",
	[-963122700] = "Hanok Village",
	[-1440159015] = "K-Pop HQ",
	[-280607009] = "K-Pop Fun Hub",
	[660463063] = "Seoul City Wall",
	[659888328] = "Seoul Gate",
	[-1688859920] = "K-Pop Concert Stage (Level 1)",
	[-1688859919] = "K-Pop Concert Stage (Level 2)",
	[-1688859918] = "K-Pop Concert Stage (Level 3)",
	[-1688859917] = "K-Pop Concert Stage (Level 4)",
	[-1688859916] = "K-Pop Concert Stage (Level 5)",
	[-1688859915] = "K-Pop Concert Stage (Level 6)",
	[-1688859914] = "K-Pop Concert Stage (Level 7)",
	[-1688859913] = "K-Pop Concert Stage (Level 8)",
	[-1688859912] = "K-Pop Concert Stage (Level 9)",
	[102197536] = "K-Pop Concert Stage (Level 10)",
	[102197537] = "K-Pop Concert Stage (Level 11)",
	[102197538] = "K-Pop Concert Stage (Level 12)",
	[102197539] = "K-Pop Concert Stage (Level 13)",
	[102197540] = "K-Pop Concert Stage (Level 14)",
	[102197541] = "K-Pop Concert Stage (Level 15)",
	[-1669151831] = "Cinco de Mayo Street & Mural",
	[-867135195] = "Eid Community Center",
	...
	[1143150652] = "Vienna St. Charles's Church",
	...
	[-1796591208] = "Gloriette View Point",
	[83636573] = "Vienna Museum Square",
	[483260335] = "Stephansdom",
	[-2064704377] = "The Gilded Supper Club",
	[1906410403] = "The Velvet Speakeasy",
	[-1212948374] = "Golden Roar Estate",
	[1774567389] = "Twenties Emporium",
	[-972576053] = "Vienna Art Museum",
	[-1167330584] = "Vienna Ferris Wheel",
	[-744338373] = "Albertina Palace",
	[-105638920] = "The Chromatic Haven",
...
presets = {
	["👑 Mayor Pass"] = {
		...
		["Season 68 - K-Pop Life"] = {
			{ id = -669830354, name = "K-Pop Park Avenue" },
			{ id = -164981350, name = "Changdeokgung Palace" },
			{ id = 1453854792, name = "Bongeunsa Temple" },
			{ id = -1440159015, name = "K-Pop HQ" },
			{ id = -280607009, name = "K-Pop Fun Hub" },
		},
		["Season 69 - Aquatic Life"] = {
			{ id = 1691892761, name = "Floating Villa" },
			{ id = -694372480, name = "River Delta" },
			{ id = 1257884349, name = "Submarine Tour" },
			{ id = 1846917630, name = "Aquatic Water Park" },
			{ id = 1966805856, name = "Aquatic Bungalows" },
		},
		["Season 70 - Vivid Vienna"] = {
			{ id = -105638920, name = "The Chromatic Haven" },
			{ id = 483260335, name = "Stephansdom" },
			{ id = -744338373, name = "Albertina Palace" },
			{ id = -972576053, name = "Vienna Art Museum" },
			{ id = -1167330584, name = "Vienna Ferris Wheel" },
		},
		["Season 71 - Roaring Twenties"] = {
			{ id = -320589832, name = "Dancing Hall" },
			{ id = -1212948374, name = "Golden Roar Estate" },
	...
	["🏫 Education"] = {
		...
		["Education - Part 4"] = {
			...
			{ id = -867135195, name = "Eid Community Center" },
		},
		["Education - Part 5"] = {
	...
	["🖼️ Landscape"] = {
		...
		["Landscape - Part 15"] = {
			...
			{ id = -108883065, name = "Turtle Sunctuary" },
			{ id = 660463063, name = "Seoul City Wall" },
			{ id = 659888328, name = "Seoul Gate" },
		},
		["Landscape - Part 16"] = {
	...
	["🎡 Entertainment"] = {
		...
		["Entertainment - Part 15"] = {
			{ id = -1980271564, name = "Aquatic Mall" },
			{ id = 102197541, name = "K-Pop Concert Stage (Level 15)" },
			{ id = -1669151831, name = "Cinco de Mayo Street & Mural" },
			...
			{ id = -1074273995, name = "Roaring Skyscraper (Level 15)" },
	...
	["⛲ Parks"] = {
		...
		["Parks - Part 27"] = {
			...
			{ id = 712194980, name = "Cherry Gazebo" },
			{ id = -1107023493, name = "Hyangwonjeong Pavilion" },
			{ id = -2136646598, name = "Cherry Park" },
			{ id = 1832050374, name = "Children's Park" },
			{ id = 1018840214, name = "Soccer Statue" },
			{ id = -1796591208, name = "Gloriette View Point" },
	...
	["🗼 Landmarks"] = {
		...
		["Landmarks - Part 11"] = {
			...
			{ id = 905448001, name = "Deoksugsung Palace" },
			{ id = -963122700, name = "Hanok Village" },
			{ id = -2072172331, name = "Gasometer Complex (Level 12)" },
			{ id = 1143150652, name = "Vienna St. Charles's Church" },
	...
	["⛱️ Beach"] = {
		...
		["Beach - Part 8"] = {
			...
			{ id = -1872807652, name = "Coral islands" },
			{ id = 2004125040, name = "Aquatic Observation Deck" },
			{ id = -1675500681, name = "Aquatic Restaurant" },
			{ id = -2075988934, name = "Aquatic Dome Resort (Level 18)" },
		},
	},
	["🗻 Mountain"] = {
		["Mountain - Part 1"] = {
	...
	["🔨 Services"] = {
		...
		["🔥 Fire"] = {
			...
			{ id = 1173563578, name = "High Tech Fire Department (Level 10)" },
			{ id = -1589439045, name = "Central Fire Station (Level 8)" },
			{ id = 1402319152, name = "Compact Fire Station (Level 5)" },
		},
		["🚓 Police"] = {
			["Police - Part 1"] = {
				...
				{ id = 1579400415, name = "Maxis HQ" },
				{ id = 637713520, name = "High Tech Police Department (Level 10)" },
				{ id = 437513969, name = "Central Police Station (Level 8)" },
				{ id = -1417983898, name = "Compact Police Station (Level 5)" },
			},
			["Police - Part 2"] = {
				{ id = 816648294, name = "Twenties Police Station (Level 10)" },
			},
		},
		["🏥 Health"] = {
			["Health - Part 1"] = {
				...
				{ id = 1316196483, name = "High Tech Medical Center (Level 10)" },
				{ id = -2044358373, name = "Compact Health Clinic (Level 5)" },
			},
			["Health - Part 2"] = {
				{ id = 1726230438, name = "Central Health Clinic (Level 8)" },
			},
		},
...
function discoverBuildingPointers()
	...
	buildingDatabase = {}
	for id, ptr in pairs(BUILDING_CACHE.data) do
		buildingDatabase[id] = { pointer = ptr, name = buildingNameById[id] or "Unknown" }
	end
	return true
end
...
function presetsMenu()
	...
	while true do
		...
		local categoryChoice = gg.choice(categoryNames, nil, "🎁 Presets")
		...
		elseif categoryChoice == 1 then
			...
			while true do
				if type(currentLevelData[1]) == "table" and currentLevelData[1].id then
					GlobalFactoryRevert(true)
					local count = #currentLevelData > 11 and 11 or #currentLevelData
					if EnsureFactorySlots(count) then
						for i, building in ipairs(currentLevelData) do
							if i <= 11 then
								local info = buildingDatabase[building.id]
								if info then
									gg.setValues({
										{
											address = FACTORY_MANAGER.slots[i].address,
											flags = gg.TYPE_QWORD,
											value = info.pointer,
										},
									})
									FACTORY_MANAGER.slots[i].currentName = building.name or info.name
								end
							end
						end
						gg.toast("✅ Preset Applied: " .. breadcrumb)
					end

					if #history > 0 then
						local lastState = table.remove(history)
						currentLevelData, breadcrumb = lastState.data, lastState.path
					else
						break
					end
				else
					...
					elseif subMenuChoice == 1 then
						...
						currentLevelData, breadcrumb = currentLevelData[selectedKey], breadcrumb .. " > " .. selectedKey
					end
				end
			end
		end
...
function factoryMethodMenu()
	while true do
		local menu = { "⬅️ Back", "🔄 Revert Factory", "🏭 Production Settings", "🎁 Presets" }
		for i = 1, 11 do
			local status = (FACTORY_MANAGER.slots[i] and FACTORY_MANAGER.slots[i].currentName)
					and ("-> " .. FACTORY_MANAGER.slots[i].currentName)
				or "-> Not Set"
			table.insert(menu, (factoryItemsMenuDefault[i] or "Slot " .. i) .. " " .. status)
		end

		local choice = gg.choice(menu, nil, "Factory Method")
		...
		elseif choice == 4 then
			...
			local idx = choice - 4
			local bld = searchBuildingByName("Select building")
			if bld then
				if EnsureFactorySlots(idx) then
					gg.setValues({
						{ address = FACTORY_MANAGER.slots[idx].address, flags = gg.TYPE_QWORD, value = bld.pointer },
					})
					FACTORY_MANAGER.slots[idx].currentName = bld.name
					gg.toast(bld.name .. " set to slot " .. idx)
				end
			end
		end
	end
		...
		if #pointerPattern > 0 then
			...
			while processedCount < requiredSlots do
				...
				if #pointerEdits > 0 then
					...
					applyRewardsToPass(
						cachedBaseAddresses[currentCell.idx],
...
function excludewar()
	...
	local offsets1 = {}
	for i, v in ipairs(results) do
		table.insert(offsets1, { address = v.address + 0x10, flags = gg.TYPE_DWORD })
	end
	gg.clearResults()
	...
	for i, v in ipairs(refined1) do
		table.insert(offsets2, { address = v.address - 0x20, flags = gg.TYPE_DWORD })
	end
	gg.clearResults()
	...
	for i, v in ipairs(found) do
		table.insert(toEdit, { address = v.address + 0x2C8, flags = gg.TYPE_DWORD })
		table.insert(toEdit, { address = v.address + 0x2D0, flags = gg.TYPE_DWORD })
		table.insert(toEdit, { address = v.address + 0x2D8, flags = gg.TYPE_DWORD })
	end
...
function findWarcards()
	...
	local check = {}
	for i, v in ipairs(results) do
		check[i] = { address = v.value + 24, flags = 32 }
	end
...
function initializeBulldozer()
	...
	local validationOffset = 0x10
	local noteOffset = -0x20
	...
	if #constantAddressesToRead > 0 then
		...
		for i, keyResult in ipairs(keyResults) do
			...
			if constantValue and constantValue ~= protectedConstant and constantValue ~= constantToIgnore then --5.0.0
...
function storesupgrade()
	local buildings = {
		...
		{ "Car Parts", { 369596261, 2142059393, 2142059394, 2142493395 } },
...
function findMasterList()
	...
	for _, res in ipairs(initialResults) do
		local ptrAddr = res.address + 0x40
	...
	for j = 1, #validationValues do
		...
		if val.value == validationQword then
			...
			sourceAddr = originalConstantAddress - 0x10
			break
		end
	end
	...
	for _, candidate in ipairs(pointerSearchResults) do
		...
		for _, ptr in ipairs(itemPointers) do
			...
			table.insert(constantsToRead, { address = ptr.value + 0x10, flags = gg.TYPE_DWORD })
		end
...
function findCombinedSpeedUpTokens()
	...
	for _, r1 in ipairs(initialResults) do
		local ptrA_addr = r1.address + 0x40
	...
	for _, r1_valid in ipairs(validStage1Results) do
		local ptrSearchSource_addr = r1_valid.address - 0x10
		gg.setVisible(false)
		gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER)
		gg.clearResults()
		...
		for _, r2 in ipairs(pointerResults) do
			...
			if ptrB_target and ptrB_target.value ~= 0 then
				local valB_addr = ptrB_target.value + 0x10
				...
				if valB_val and valB_val.value == -1247906808 then
					...
					if ptrC_target and ptrC_target.value ~= 0 then
						local valC_addr = ptrC_target.value + 0x10
...
function findExpansionCertificates()
	...
	for _, result in ipairs(initialResults) do
		local ptrAddr = result.address + 0x40
		...
		if ptrVal and ptrVal.value ~= 0 then
			...
			if valVal and valVal.value == 7022359169637113716 then
				local searchSrcAddr = result.address - 0x10
				gg.setVisible(false)
				gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER)
				gg.clearResults()
	...
	for _, p in ipairs(candidatePointers) do
		...
		if pTarget1 and pTarget1.value ~= 0 then
			local valAddr1 = pTarget1.value + 0x10
			...
			if valVal1 and valVal1.value == -869378290 then
				...
				if pTarget2 and pTarget2.value ~= 0 then
					local valAddr2 = pTarget2.value + 0x10
...
function findExpansionItems()
	...
	local VALIDATOR_OFFSET_FROM_POINTER_TARGET = 0x10
	...
	for _, res in ipairs(initial_results) do
		local check_val = gg.getValues({ { address = res.address + 0x10, flags = gg.TYPE_DWORD } })[1]
		if check_val and check_val.value == 25 then
			local key_val = gg.getValues({ { address = res.address - 0x10, flags = gg.TYPE_QWORD } })[1]
...
function executeChange(itemsToApply)
	...
	for i = 1, count do
		table.insert(
			writes,
			{ address = FACTORY_MANAGER.slots[i].address, value = itemsToApply[i].value, flags = gg.TYPE_QWORD }
		)
		FACTORY_MANAGER.slots[i].currentName = itemsToApply[i].name
	end
	gg.setValues(writes)
	gg.toast("Applied " .. count .. " items")
end
...
function showWarPresets()
	...
	local attacks = {
		...
		{ "❄️ Blizzaster", { "Propeller", "Ammo", "Duck" } },
	...
	while true do
		...
		elseif choice == 1 then
			...
			local attackName = selectedAttack[1]
			local itemsToApply = {}

			if attackName == "🦆 Doomsday Quack" then
				if not findMasterList() then
					goto next_choice
				end
				table.insert(itemsToApply, cache.warItems["Duck"])
				local masterItem = gg.getValues({ cache.constantMap[masterListConstants[42]] })[1]
				if masterItem then
					masterItem.name = "Eiffel Tower"
					table.insert(itemsToApply, masterItem)
				end
			else
				for _, itemName in ipairs(selectedAttack[2]) do
					local item = cache.warItems[itemName]
					if item then
						table.insert(itemsToApply, item)
					end
				end
			end
			...
			if EnsureFactorySlots(count) then
				...
				for i = 1, count do
					...
					local slot = FACTORY_MANAGER.slots[i]
					if currentItem and slot then
						table.insert(
							writes,
							{ address = slot.address, value = currentItem.value, flags = gg.TYPE_QWORD }
						)
						slot.currentName = currentItem.name
					end
				end

				if #writes > 0 then
					gg.setValues(writes)
					gg.toast("✅ Applied: " .. attackName)
				end
			end
		end
		::next_choice::
...
function skipTutorial()
	...
	gg.setVisible(false)
	gg.clearResults()
	gg.alert("Done, Restart the game.")
end

function maxmaterialstorage()
	local alertMsg = [[Please make sure you do both before confirming:

1- Bulldoze your current Material Storage (use our function Bulldoze/store all buildings inside building tool)

2- Place the "Royal Blimp" building (it's inside parks section)

obs: you might need to go Daniel twice before it takes effect)]]

	local choice = gg.alert(alertMsg, "Cancel", "Confirm")

	if choice ~= 2 then
		return
	end

	local pondID = "527731446"
	local storage596ID = "-2146309853"

	gg.clearResults()
	gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER)
	gg.setVisible(false)
	gg.searchNumber(pondID, gg.TYPE_DWORD)

	local count = gg.getResultsCount()
	if count == 0 then
		gg.setVisible(true)
		gg.alert("Error: Pond not found. Make sure you placed it on the map.")
		return
	end

	gg.getResults(count)
	gg.editAll(storage596ID, gg.TYPE_DWORD)

	gg.clearResults()
	gg.toast("✅ Success! Go to Daniel's city and come back.")
end
...
function storageWipe_findRegularItems()
	...
	for i, res in ipairs(initialResults) do
		table.insert(batchReads, { address = res.address + 0x10, flags = gg.TYPE_DWORD })
		table.insert(batchReads, { address = res.address - 0x10, flags = gg.TYPE_DWORD })
	end
	...
	for i, res in ipairs(idResults) do
		table.insert(finalEditList, { address = res.address + 0x10, flags = gg.TYPE_DWORD })
	end
...
function storageWipe_findWarItems()
	...
	for _, res in ipairs(initial_results) do
		local check = gg.getValues({ { address = res.address + 0x10, flags = gg.TYPE_DWORD } })[1]
		if check and check.value == 52 then
			local key = gg.getValues({ { address = res.address - 0x10, flags = gg.TYPE_QWORD } })[1]
	...
	for _, s in ipairs(structures) do
		local res = gg.getValues({ { address = s.address + 0x10, flags = gg.TYPE_DWORD } })[1]
...
function Nuke_findBuildingRoots()
	...
	for _, anchor_candidate in ipairs(results) do
		...
		if ptr_A_val and ptr_A_val.value ~= 0 then
			local validation_addr = ptr_A_val.value + 0x10
	...
	local building_roots = {
		Residential = other_root_ptr + 0x830,
		Other = other_root_ptr,
		Beach = other_root_ptr - 0x1500,
		Mountain = other_root_ptr - 0x11A8,
	}
...
function Nuke_execute()
	...
	gg.setVisible(true)
	gg.alert("Nuke successful! Restart the game.")
end
...
function MainRegion()
	...
	gg.clearResults()
	gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER)
	gg.searchNumber("6553796", gg.TYPE_DWORD)
	gg.setVisible(false)
	...
	local list = {
		{ n = "Capital City", v = 6553796, s = false, b = {} },
...
function activateNegativeStorageAndMetal()
	...
	for _, res in ipairs(initialResults) do
		local checkValue = gg.getValues({ { address = res.address + 0x10, flags = gg.TYPE_DWORD } })[1]
		if checkValue and checkValue.value == 15 then
			savedAddress = checkValue.address - 0x20
			break
		end
	end
...
function revertMethod2()
	...
	for _, res in ipairs(initialResults) do
		...
		if check and check.value == 15 then
			search_key_addr = check.address - 0x20
			break
		end
	end
	...
	local valuesToEdit = {}
	for _, res in ipairs(finalResults) do
		table.insert(valuesToEdit, { address = res.address + 0x10, flags = gg.TYPE_DWORD, value = 667465812 })
	end
	gg.setValues(valuesToEdit)
...
function findBuildingAnchors()
	...
	for _, anchor_candidate in ipairs(results) do
		...
		if ptr_A_val_table and ptr_A_val_table[1] and ptr_A_val_table[1].value ~= 0 then
			local validation_addr = ptr_A_val_table[1].value + 0x10
	...
	if other_anchor_ptr_table and other_anchor_ptr_table[1] and other_anchor_ptr_table[1].value ~= 0 then
		...
		cache.residential_base_anchor = other_anchor_ptr + 0x830
...
function performMoveOperation(anchor_base_address, group_name, newX, newY)
	...
	for i, instance in ipairs(building_instances) do
		...
		if structureAddr and structureAddr ~= 0 then
			local buildingId = gg.getValues({ { address = structureAddr + 0x10, flags = gg.TYPE_DWORD } })[1].value
...
local war_cards_ids = {
	{ name = "📘 Comic Hand", id = -642931401 },
	{ name = "🧲 Magnetism", id = -408119043 },
	{ name = "🌿 Plant Monster", id = -2043179032 },
	{ name = "🐍 Hissy Fit", id = -477763312 },
	{ name = "🎺 Mellow Bellow", id = -438627919 },
	{ name = "⚡ Electric Deity", id = -516898705 },
	{ name = "🦆 Doomsday Quack", id = -556034098 },
	{ name = "🛡️ Shield Buster", id = 500621513 },
	{ name = "⚖️ 16 Tons", id = -1847502067 },
	{ name = "🐙 Tentacle Vortex", id = -447254436 },
	{ name = "🖐️ Hands of Doom", id = -329848257 },
	{ name = "🔬 Shrink Ray", id = -525525222 },
	{ name = "🗿 Giant Rock Monster", id = -595169491 },
	{ name = "🌪️ Not In Kansas", id = -486389829 },
	{ name = "🩰 Dance Shoes", id = -2004043639 },
	{ name = "🏺 Ancient Curse", id = -1886637460 },
	{ name = "❄️ Blizzaster", id = -368983650 },
	{ name = "🤖 Vu Robot", id = -751711063 },
	{ name = "🚪 Building Portal", id = -1769231281 },
	{ name = "🎬 B Movie Monster", id = -712575670 },
	{ name = "🐟 Fishaster", id = -1925772853 },
	{ name = "🕷️ Spiders", id = -1730095888 },
	{ name = "🪩 Disco Twister", id = -2082314425 },
	{ name = "🛸 Zest From Above", id = 539756906 },
}

factory_cache = {}

function execute_card_injection(selected_names)
	GlobalFactoryRevert(true)
	local count = #selected_names
	if not EnsureFactorySlots(count) then
		return
	end
	if not map_war_cards_memory() then
		return
	end
	local edits = {}
	for i = 1, count do
		local ptr = war_cards_ptr[selected_names[i]]
		if ptr then
			table.insert(edits, { address = FACTORY_MANAGER.slots[i].address, flags = gg.TYPE_QWORD, value = ptr })
			FACTORY_MANAGER.slots[i].currentName = selected_names[i]
		end
	end
	gg.setValues(edits)
	gg.toast("Cards Applied")
end

function map_war_cards_memory()
	if is_cards_mapped then
		return true
	end

	gg.toast("Mapping War Cards...")

	gg.setRanges(gg.REGION_C_ALLOC | gg.REGION_ANONYMOUS | gg.REGION_OTHER)

	gg.clearResults()
	gg.setVisible(false)
	gg.searchNumber("-642931401", gg.TYPE_DWORD)

	local initial = gg.getResults(gg.getResultsCount())
	gg.setVisible(false)

	if #initial == 0 then
		gg.alert("War Cards not found")
		return false
	end
	...
	for _, v in ipairs(initial) do
		local check = gg.getValues({ { address = v.address + 0x10, flags = gg.TYPE_DWORD } })[1]

		if check and check.value == 62 then
			local k = gg.getValues({ { address = v.address - 0x10, flags = gg.TYPE_QWORD } })[1]

			if k and k.value ~= 0 then
				key = k.value
				break
			end
		end
	...
	if not key then
		gg.alert("War Card validation failed")
		return false
	end

	gg.clearResults()
	gg.searchNumber(tostring(key), gg.TYPE_QWORD)
	...
	for _, c in ipairs(war_cards_ids) do
		id_to_name[c.id] = c.name
	end

	for _, s in ipairs(structures) do
		local id_val = gg.getValues({ { address = s.address + 0x10, flags = gg.TYPE_DWORD } })[1].value

		local name = id_to_name[id_val]

		if name then
			war_cards_ptr[name] = s.address
		end
	end

	is_cards_mapped = true
...
function inject_cards_menu()
	if not map_war_cards_memory() then
		return
	end

	while true do
		local menu = { "🔙 Back", "♻️ Revert Factory" }

		for _, c in ipairs(war_cards_ids) do
			table.insert(menu, c.name)
		end

		local choice = gg.multiChoice(menu, nil, "🃏 Select War Cards (Max 11)")
		...
		for i, v in pairs(choice) do
			if v and i > 2 then
				table.insert(selected, war_cards_ids[i - 2].name)
				count = count + 1
			end
		end

		if count > 11 then
			gg.alert("Maximum 11 cards allowed")
		else
			execute_card_injection(selected)
		end
	end
end

function war_cards_menu()
	inject_cards_menu()
end
...
local b1 = { 0x65746908, 0x0000006D, 0x00000000, 0x00000000, 0x00000000, 0x00000000 }
...
local currencyData = {
	comic_hand = {
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C31, 0x00006C65, 0x00000000 },
		},
	},
	magnetism = {
		name = "🧲 Magnetism",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C37, 0x00006C65, 0x00000000 },
		},
	},
	plant_monster = {
		name = "🌿 Plant Monster",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3131, 0x006C6576, 0x00000000 },
		},
	},
	hissy_fit = {
		name = "🐍 Hissy Fit",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3832, 0x006C6576, 0x00000000 },
		},
	},
	mellow_bellow = {
		name = "📢 Mellow Bellow",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3932, 0x006C6576, 0x00000000 },
		},
	},
	electric_deity = {
		name = "⚡ Electric Deity",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3732, 0x006C6576, 0x00000000 },
		},
	},
	doomsday_quack = {
		name = "🦆 Doomsday Quack",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3632, 0x006C6576, 0x00000000 },
		},
	},
	shield_buster = {
		name = "🛡️ Shield Buster",
		isSpecial = true,
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3033, 0x006C6576, 0x00000000 },
		},
	},
	sixteen_tons = {
		name = "⚖️ 16 Tons",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3631, 0x006C6576, 0x00000000 },
		},
	},
	tentacle_vortex = {
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C36, 0x00006C65, 0x00000000 },
		},
	},
	hands_of_doom = {
		name = "💀 Hands of Doom",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C39, 0x00006C65, 0x00000000 },
		},
	},
	shrink_ray = {
		name = "🔫 Shrink Ray",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C34, 0x00006C65, 0x00000000 },
		},
	},
	giant_rock_monster = {
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3532, 0x006C6576, 0x00000000 },
		},
	},
	not_in_kansas = {
		name = "🌪️ Not In Kansas",
		isSpecial = true,
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C35, 0x00006C65, 0x00000000 },
		},
	},
	dance_shoes = {
		name = "👟 Dance Shoes",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3231, 0x006C6576, 0x00000000 },
		},
	},
	ancient_curse = {
		name = "🏺 Ancient Curse",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3531, 0x006C6576, 0x00000000 },
		},
	},
	blizzaster = {
		name = "❄️ Blizzaster",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615722, 0x61736944, 0x72657473, 0x76654C38, 0x00006C65, 0x00000000 },
		},
	},
	vu_robot = {
		name = "🤖 Vu Robot",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3132, 0x006C6576, 0x00000000 },
		},
	},
	building_portal = {
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3831, 0x006C6576, 0x00000000 },
		},
	},
	b_movie_monster = {
		name = "👾 B Movie Monster",
		isSpecial = true,
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3232, 0x006C6576, 0x00000000 },
		},
	},
	fishaster = {
		name = "🐟 Fishaster",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3431, 0x006C6576, 0x00000000 },
		},
	},
	spiders = {
		name = "🕷️ Spiders",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3931, 0x006C6576, 0x00000000 },
		},
	},
	disco_twister = {
		name = "🪩 Disco Twister",
		...
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3031, 0x006C6576, 0x00000000 },
		},
	},
	zest_from_above = {
		name = "🍋 Zest From Above",
		isSpecial = true,
		hexCodes = {
			block1 = b1,
			block2 = { 0x72615724, 0x61736944, 0x72657473, 0x654C3133, 0x006C6576, 0x00000000 },
		},
	},
}
...
local function applyRewardsToCell(baseAddress, cellInfo, selectedCurrencyKeys, amounts)
	...
	for i = 1, #list, 3 do
		local reward_counter = ((i - 1) / 3) + 1
		local cycle_index = ((reward_counter - 1) % #selectedCurrencyKeys) + 1
		local currentKey = selectedCurrencyKeys[cycle_index]
		local currency = currencyData[currentKey]
		local ptr1, ptr2 = resolvedPointers[pointer_idx].value, resolvedPointers[pointer_idx + 1].value
		pointer_idx = pointer_idx + 2
		if ptr1 and ptr1 ~= 0 and ptr2 and ptr2 ~= 0 then
...
function mainMenuOOO2()
	cachedBaseAddresses = GetVuPassBases()
	if not cachedBaseAddresses then
		return
	end
	while true do
		local menuItems, multiMenuKeys = { "🔙 Back" }, {}
		for _, key in ipairs(currencyDisplayOrder) do
			table.insert(menuItems, currencyData[key].name)
			table.insert(multiMenuKeys, key)
		end
		local selection = gg.multiChoice(menuItems, nil, "Select cards (Max 10):")
		if selection == nil then
			break
		elseif selection[1] then
			return
			...
			local chosenKeys = {}
			for index, checked in pairs(selection) do
				if checked and index >= 2 then
					table.insert(chosenKeys, multiMenuKeys[index - 1])
				end
			end
			if #chosenKeys == 0 then
				gg.toast("No cards selected.")
			elseif #chosenKeys > 10 then
				gg.alert("❌ Max 10 cards allowed.")
			else
				local inputs = gg.prompt({ "How much level do you want?" }, { "19" }, { "number" })
				if inputs then
					local level = tonumber(inputs[1])
					if not level or level < 0 then
						...
						table.insert(cellMenu, "✔️ All Cells")
						table.insert(cellMenu, "🔙 Back")
						local cellChoice = gg.choice(cellMenu, nil, "Select cell:")
						if cellChoice then
							gg.setVisible(false)
							if cellChoice == #cellMenu then
							elseif cellChoice == #cellMenu - 1 then
								...
								applyRewardsToCell(
									cachedBaseAddresses[cellChoice],
									cellData[cellChoice],
									chosenKeys,
									amounts
								)
...
local educationBuildingData = {
	{ id = 448264947, name = "Residential Zone Slot" },
	{ id = 385991488, name = "Cozy Homes Zone Slot" },
	{ id = 1581365320, name = "Kyoto Zone Slot" },
	{ id = 562571616, name = "Florentine Zone Slot" },
	{ id = -979789707, name = "Art Nouveau Zone Slot" },
	{ id = 1647433881, name = "Old Town Zone Slot" },
}
	...
	if mainCat.isEpicCategory then
		for _, subCat in ipairs(mainCat.categories) do
			for _, tier in ipairs(subCat.tiers) do
				buildingNameById[tier.id] = tier.name
			end
		end
	else
...
function ensureEducationBuildingsAreFound(neededCount)
	...
	for i = #cachedBuildings + 1, neededCount do
		...
		gg.toast("⏳ Locating " .. target.name .. "...")
		local ptr = discoveredPointers[target.id]
		local wasFound = false

		if ptr and ptr ~= 0 then
			gg.clearResults()
			gg.searchNumber(tostring(ptr), gg.TYPE_QWORD)
			local found = gg.getResults(2000)

			for _, r in ipairs(found) do
				local val = gg.getValues({ { address = r.address - 0x78, flags = gg.TYPE_QWORD } })[1].value
				if val == 7306080439843839784 then
					local currentPtrValue = gg.getValues({ { address = r.address, flags = gg.TYPE_QWORD } })[1].value
					cachedBuildings[i] = { address = r.address, originalValue = currentPtrValue, name = target.name }
					gg.toast("✅ Found " .. target.name)
					wasFound = true
					break
				end
			end
		...
		if not wasFound then
			gg.setVisible(true)
			gg.alert("❌ Building not found: " .. target.name)
			return false
		end
	end
...
function revertAllChanges()
	if #cachedBuildings == 0 then
		gg.toast("No changes to revert.")
		return
	end

	local setTable = {}
	for _, b in ipairs(cachedBuildings) do
		table.insert(setTable, { address = b.address, value = b.originalValue, flags = gg.TYPE_QWORD })
	end
	gg.setValues(setTable)
	gg.toast("✅ Reverted all.")
end
...
function showTierMenu(zone)
	while true do
		...
		local choice = gg.multiChoice(menu, nil, "Select max 6 tiers for " .. zone.name)
		...
		elseif choice[1] then
			...
			for i, s in pairs(choice) do
				if s and i > 1 then
					table.insert(selected, zone.tiers[i - 1])
				end
			end

			if #selected > 6 then
				gg.alert("❌ Error: Select a maximum of 6 items!")
...
function showEpicCategoryMenu()
	while true do
		...
		local choice = gg.choice(menu, nil, "Epic Categories")
...
function instantResidencesMenu()
	...
	while true do
		local menu = { "🔙 Back", "↩️ Revert All" }
		for _, d in ipairs(buildingData) do
			table.insert(menu, d.name)
		end

		local choice = gg.choice(menu, nil, "Select Zone Type")
...
function menup()
	gg.hideUiButton()
	while true do
		local titleHeader = "• Main Menu • | Script by Isaac and Kiro - 5.0.0"
		local menuItens = {
			"🚪 Exit Script",
			"📄 View Limits",
			"💰 Currencies",
			...
			"🏢 Building Class",
			"🪟 View & UI Control",
			populationEnabled and ("👤 Population 🟢 (" .. populationValue .. ")") or "👤 Population 🔴",
			"🗃️ Depot Class",
			"📦 Storage Class",
			"🌎 Unlock All Regions",
		...
		elseif opcao == 1 then
			local confirm =
				gg.choice({ "✅ Yes, exit", "❌ Cancel" }, nil, "Are you sure you want to exit the script?")
			if confirm == 1 then
				local info = gg.multiChoice(
					{
						"©️ Copy Discord Link",
						"📋 Copy WhatsApp Number",
					},
					nil,
					"Thank you for using the SCBI Combo Script!\n\nAuthors: Isaac and Kiro\n\nJoin our discord server at https://discord.gg/scbicombo\n\nYou may also contact +1 840-250-9300 on WhatsApp for more information and a link to the server."
				)

				if info ~= nil then
					if info[1] then
						gg.copyText("https://discord.gg/scbicombo")
						gg.toast("Discord link copied to clipboard!")
					end
					if info[2] then
						gg.copyText("+1 840-250-9300")
						gg.toast("WhatsApp number copied to clipboard!")
					end
					os.exit()
				end
			end
